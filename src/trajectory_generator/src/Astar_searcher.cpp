...
double Astarpath::getHeu(MappingNodePtr node1, MappingNodePtr node2) {
  ...
  // 使用欧几里得距离作为启发式函数
  Vector3d pos1 = node1->coord;
  Vector3d pos2 = node2->coord;

  // 欧几里得距离
  double euclidean_distance = (pos1 - pos2).norm();

  // tie_breaker: 用于减少估计值相等情况下的搜索范围
  // tie_breaker 的值通常是一个小的正数，确保在路径选择时更倾向于直线距离更小的路径
  double tie_breaker = 1.0 + 1e-6;

  // 启发式估计值
  double heu = euclidean_distance * tie_breaker;
  ...
  while (!Openset.empty()) {
    ...
    for (unsigned int i = 0; i < neighborPtrSets.size(); i++) {
        ...
        if (neighborPtr->id == 0) {
            ...
            neighborPtr->id = 1; // 标记为已加入 OpenSet
        }
        // 如果节点已在 OpenSet 中
